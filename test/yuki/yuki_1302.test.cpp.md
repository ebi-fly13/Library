...
data:
  ...
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/1302\"\n\n#include <iostream>\n\
    ...
    \ >> n;\n    FPS f(n + 1);\n    Binomial::reserve(n);\n    for (int i = 0; i <\
    \ n + 1; i++) {\n        f[i] = (i + 1) * Binomial::inv_f(i);\n    }\n    mint\
    \ ans = f.pow(n)[n - 2] * Binomial::f(n - 2) / mint(n).pow(n - 2);\n    std::cout\
    \ << ans.val() << '\\n';\n}"
  dependsOn:
  - convolution/ntt.hpp
  ...
  verifiedWith: []
documentation_of: test/yuki/yuki_1302.test.cpp
layout: document
redirect_from:
- /verify/test/yuki/yuki_1302.test.cpp
- /verify/test/yuki/yuki_1302.test.cpp.html
