...
struct submodular {
public:
    submodular(int n, int add_vertex = 0) : gh(n + add_vertex +2), s(n), t(n+1), max(n+add_vertex+2),u(n+2)  { }
    ...
    void add_three_variable_function(int i, int j, int k, Cap a, Cap b, Cap c, Cap d, Cap e, Cap f, Cap g, Cap h) {
        assert(u < max);
        Cap p = (a + d + f + g) - (b + c + e + h);
        if(p >= 0) {
            Cap p1 = f-b, p2 = g-e, p3 = d-c;
            Cap p23 = (b+c)-(a+d), p31 = (b+e)-(a+f), p12 = (c+e)-(a+g);
            R += a;
            ...
            Cap p32 = (f+g)-(e+h), p13 = (d+g)-(c+h), p21 = (d+f)-(b+h);
            R += h;
